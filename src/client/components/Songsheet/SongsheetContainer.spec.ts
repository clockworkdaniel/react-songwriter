...
describe('Songsheet integration tests', () => {
  ...
  describe('without dispatching fetch in componentDidMount', () => {
    ...
    store = setupStore();
    const wrapper = render(<MockRouter><Provider store={store}><SongsheetContainer match={match} /></Provider></MockRouter>);
  ...
  describe('as a result of fetch', () => {
    ...
    const wrapper = mount(
      <MockRouter>
        <Provider store={store}><SongsheetContainer match={match} /></Provider>
      </MockRouter>
    );

    it('should render the structure', () => {
      wrapper.update();
      expect(wrapper.find('.songsheet__structure').children().length).toBeGreaterThan(0);
    });
