...
type SongsheetState = {
  ...
  textBeingEditedPathArray: any[];
  song: Song;
};

const songsheetReducer: LoopReducer<SongsheetState, any> = (
  state = initialState,
  action
) => {
  switch (action.type) {
    ...
    case "FETCH_SONG_SUCCESS": {
      return update(state, {
        ...
        uiState: { isEditable: { $set: action.res.isEditable } }
      });
    }
    ...
    case "SWITCH_PRIVACY_SUCCESS": {
      return update(state, {
        song: { isPublic: { $set: action.res.isPublic } }
      });
    }
    ...
    case "UPDATE_CHORD": {
      ...
      // change chord on per word basis
        ...
        for (
          ...
          i > -1 && lineCopy.characters[i].character !== " ";
          i--
        ) {
        ...
        for (let i = action.characterKey + 1, end = false; end === false; i++) {
          if (lineCopy.characters[i]) {
            ...
            !lineCopy.characters[i] || lineCopy.characters[i].character === " "
          );
        }
      }
      ...
      return update(state, {
        song: {
          structure: {
            [action.sectionKey]: {
              lines: { [action.lineKey]: { $set: lineCopy } }
            }
          }
        }
      });
      ...
      if (!state.uiState.chordMode) {
        songCopy.structure.map((section, index) => {
          ...
          section.lines.map((line, index) => {
            ...
            songCopy.structure[sectionKey].lines[
              index
    ...
    case "UPDATE_CHORD_TO_PAINT": {
      return update(state, {
        uiState: { chordToPaint: { $set: action.chord } }
      });
    }

    case "UPDATE_PAINT_SPECIFICITY": {
      return update(state, {
        uiState: { paintSpecificity: { $set: action.newSpecificity } }
      });
    }
    ...
    case "DELETE_SECTION": {
      return update(state, {
        song: { structure: { $splice: [[action.sectionKey, 1]] } }
      });
    }

    case "DUPLICATE_SECTION": {
      const sectionCopy = JSON.parse(
        JSON.stringify(state.song.structure[action.sectionKey])
      );
      return update(state, {
        song: { structure: { $splice: [[action.sectionKey, 0, sectionCopy]] } }
      });
    }
    ...
    case "NEW_LINE": {
      ...
      const startPosition = blankLineBehind
        ? action.lineKey
        : action.lineKey + 1;
    ...
    case "SPLIT_LINE": {
      ...
      const leadingSpace =
        lines[action.lineKey].fullLine.charAt(caretPosition) === " ";
      const trailingSpace =
        lines[action.lineKey].fullLine.charAt(caretPosition - 1) === " ";
      ...
      const afterSplit = {
        fullLine: lines[action.lineKey].fullLine.substring(
          afterSplitPos,
          lineLength
        ),
      ...
      afterSplit.fullLine =
        afterSplit.fullLine.slice(0, 1).toUpperCase() +
        afterSplit.fullLine.slice(1);
      afterSplit.characters =
        leadingSpace || trailingSpace
          ? afterSplit.characters.splice(1)
          : afterSplit.characters;
        ...
        song: {
          structure: {
            [action.sectionKey]: {
              lines: {
                $splice: [
                  [action.lineKey, 1, beforeSplit],
                  [action.lineKey + 1, 0, afterSplit]
                ]
              }
            }
          }
    ...
    case "DELETE_LINE": {
      ...
      const newCaretPosition =
        section.lines[action.lineKey - 1].fullLine.length;
    ...
    case "JOIN_LINES": {
      ...
      const thisLinePrepared = prevLineIsEmpty
        ? thisLine.fullLine.slice(0, 1).toUpperCase() +
          thisLine.fullLine.slice(1)
        : thisLine.fullLine.slice(0, 1).toLowerCase() +
          thisLine.fullLine.slice(1);

      // join line on to end of previous one
      const joinedLine = {
        fullLine:
          prevLine.fullLine + (prevLineIsEmpty ? "" : " ") + thisLinePrepared,
        characters: prevLine.characters.concat(
          { character: " ", chord: lastChord(prevLine) },
          thisLine.characters
        )
      };
    ...
    case "UPDATE_EDITED_TEXT": {
      ...
      let newObject: Object = { $set: action.committedTextObj };
